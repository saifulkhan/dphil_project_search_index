...
Treemap::Treemap()
{
    ...
    // Scan cache
    m_rootNodeTM = m_cacheParser->parse();
}
...
string Treemap::treemap(const int x, const int y, const int width, const int height)
{
    /*
     * When scale, height, width etc changed by control window only new layout will be done,
     * whre the NodeTM.rectF will only change
     */
    m_layout.doLayout(m_rootNodeTM, RectF(x, y, width, height));
    m_coordinate.clear();
...
string Treemap::treemap(const int depth)
{
    if (depth < 3)
        return "";
    json_spirit::Array output;
