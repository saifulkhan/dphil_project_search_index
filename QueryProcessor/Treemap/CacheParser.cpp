...
NodeTM* CacheParser::parse(string rootDir)
{
    ...
    vector<FileInfo> fileInfoVec = cache->read();

    for(FileInfo fileInfo : fileInfoVec)
    {
        vector<string> list = tokenisePath(rootDir, fileInfo.m_path);
        if (list.empty())
            continue;

        NodeTM* parent_of_leaf_node = updateTree(m_rootNode, list);
...
NodeTM* CacheParser::updateTree(NodeTM* parent, vector<string> list)
{
    #ifdef DBG_CACHEPARSER
    ...
    if (!list.empty())
    {
        ...
        parent = this->updateTree(node, list);
    }
    return parent;
}
...
vector<string> CacheParser::tokenisePath(const string &rootDirPath, const string& filePath)
{
    vector<string> list;
    string str = filePath;
...
/*********************************************************************************************************
 * NOT USED,
 * TODO: Safety check the last character if "/", then delete.
 *********************************************************************************************************/
