#include "load_wordnet.h"

#include <fstream>
#include <iostream>
#include <sstream>
#include <stdexcept>
#include <algorithm>

#include <boost/graph/adjacency_list.hpp>
#include <boost/progress.hpp>

#include "Wordnet.h"
#include "info_helper.h"
#include "pos_t.h"

//namespace bg = boost::graph;

namespace WN
{

// Load synset's words
void load_data_row_words(stringstream& srow, synset& synset, Wordnet &wn)
{
    srow >> hex >> synset.w_cnt >> dec;
    for (int i = 0; i < synset.w_cnt; i++)
    {
        //word lex_id

        string word;
        srow >> word;
        synset.words.push_back(word);

        int lex_id;
        srow >> hex >> lex_id >> dec;
        synset.lex_ids.push_back(lex_id);

    }

    wn.offsetWordList[synset.id] = synset.words;
}

// Add rel to graph
void add_wordnet_rel(string& pointer_symbol_,// type of relation
                         int synset_offset,           // dest offset
                         pos_t pos,                   // p.o.s. of dest
                         int src,                     // word src
                         int trgt,                    // word target
                         synset& synset,              // source synset
                         Wordnet& wn,                 // our Wordnet
                         info_helper& info)           // helper
{
      //if (pos == S || synset.pos == S)
      //  return; //FIXME: check where are s synsets.

      int u = synset.id;
      int v = info.compute_indice(synset_offset, pos);

      Ptr p;
      p.pointer_symbol = pointer_symbol_;
      p.source = src;
      p.target = trgt;


      //boost::add_edge(u,v, p, wn.wordnet_graph);
}


// load Ptrs
void load_data_row_ptrs(stringstream& srow, synset& synset, Wordnet& wn, info_helper& info)
{
    vector <Ptr> ptrvec;

    srow >> synset.p_cnt;
    for (int i = 0; i < synset.p_cnt; i++)
    {
        //http://wordnet.princeton.edu/wordnet/man/wndb.5WN.html#sect3
        //pointer_symbol  synset_offset  pos  source/target
        string pointer_symbol_;
        int   synset_offset;
        pos_t pos;
        int   src;
        int   trgt;

        srow >> pointer_symbol_;
        srow >> synset_offset;

        char c;
        srow >> c;
        pos = info.get_pos(c);

        //print extracted edges
        //cout << "(" << pointer_symbol << ", " << synset_offset;
        //cout << ", " << pos << ")" << endl;

        // Extract source/target words info
        string src_trgt;
        srow >> src_trgt;
        stringstream ssrc(string(src_trgt,0,2));
        stringstream strgt(string(src_trgt,2,2));
        ssrc >> hex >> src >> dec;
        strgt >> hex >> trgt >> dec;

        //add_wordnet_rel(pointer_symbol_, synset_offset, pos, src, trgt, synset, wn, info);

        Ptr p;
        p.pointer_symbol = pointer_symbol_;
        p.synsetid = info.compute_indice(synset_offset, pos);
        p.source = src;
        p.target = trgt;

        ptrvec.push_back(p);
    }

    wn.offsetPtrList[synset.id] = ptrvec;
}


// Load a synset and add it to the wordnet class.
// http://wordnet.princeton.edu/wordnet/man/wndb.5WN.html#sect3
// synset_offset  lex_filenum  ss_type  w_cnt  [word  lex_id, ...]  p_cnt  [ptr...]  [frames...]  |   gloss

void load_data_row(const string& row, Wordnet& wn, info_helper& info)
{
      synset synset;

      stringstream srow(row);
      srow >> synset.synset_offset_;
      srow >> synset.lex_filenum;
      srow >> synset.ss_type_;

      // extra information
      synset.pos = info.get_pos(synset.ss_type_);
      //if (synset.pos == S)  return; //FIXME: Check where are s synsets
      synset.id  = info.compute_indice(synset.synset_offset_, synset.pos);

      // words
      load_data_row_words(srow, synset, wn);

      // ptrs
      load_data_row_ptrs(srow, synset, wn, info);

      //frames (skipped)
      string tmp;
      while (srow >> tmp)
        if (tmp == "|")
          break;

      //gloss (could be nicer)
      while (srow >> tmp)
      {
        synset.gloss.append(tmp);
        synset.gloss.append(" ");
      }

      // Add synset to graph
      //wn.wordnet_graph[synset.id] = synset;
}


    // Parse data.noun files
    void load_wordnet_data(const string& fn, Wordnet& wn, info_helper& info)
    {
      ifstream fin(fn.c_str());
      if (!fin.is_open())
        throw runtime_error("File missing: " + fn);

      static const int MAX_LENGTH = 20480;
      char row[MAX_LENGTH];

      //skip header
      for(unsigned i = 0; i < 29; i++)
        fin.getline(row, MAX_LENGTH);

      //parse data line
      while (fin.getline(row, MAX_LENGTH))
        load_data_row(row, wn, info);

      fin.close();
    }


    //FIXME: It seems possible to replace synset_offsets with indice here.
    void load_index_row(const string& row, Wordnet& wn, info_helper& info)
    {
      // lemma  pos  synset_cnt  p_cnt  [ptr_symbol...]  sense_cnt  tagsense_cnt   synset_offset  [synset_offset...]
      Index index;
      stringstream srow(row);

      srow >> index.lemma;
      srow >> index.pos_;
      srow >> index.synset_cnt;
      srow >> index.p_cnt;


      string tmp_p;
      for (int i = 0; i < index.p_cnt; i++) {
        srow >> tmp_p;
        index.ptr_symbols.push_back(tmp_p);
      }
      srow >> index.sense_cnt;
      srow >> index.tagsense_cnt;

      int tmp_o;
      while (srow >> tmp_o)
        index.synset_offsets.push_back(tmp_o);

      //extra data
      index.pos = info.get_pos(index.pos_);

      //add synset to index list
      wn.indexList.push_back(index);

      //cout << __PRETTY_FUNCTION__ << index.lemma << ", " << index.p_cnt << ", " << index.synset_offsets.size() << endl;

    }


    void load_wordnet_index(const string& fn, Wordnet& wn, info_helper& info)
    {
      ifstream fin(fn.c_str());
      if (!fin.is_open())
        throw runtime_error("File Not Found: " + fn);

      static const int MAX_LENGTH = 20480;
      char row[MAX_LENGTH];

      //skip header
      for(unsigned i = 0; i < 29; i++)
        fin.getline(row, MAX_LENGTH);

      //parse data line
      while (fin.getline(row, MAX_LENGTH))
        load_index_row(row, wn, info);

      fin.close();
    }


    void load_wordnet_exc(const string& dn, string cat, Wordnet& wn, info_helper&)
    {
      string fn = dn + cat + ".exc";
      ifstream fin(fn.c_str());
      if (!fin.is_open())
        throw runtime_error("File Not Found: " + fn);

      map<string, string>& exc = wn.exc[get_pos_from_name(cat)];

      static const int MAX_LENGTH = 2048;
      char row[MAX_LENGTH];

      while (fin.getline(row, MAX_LENGTH)) {
        stringstream srow(row);
        string key, value;
        srow >> key;
        srow >> value;
        //cout << "Debug: " << __PRETTY_FUNCTION__ << key << ", " << value << "\n";
        exc[key] = value;
      }
    }

    void load_wordnet_cat(const string dn, string cat, Wordnet& wn, info_helper& info)
    {
      load_wordnet_data((dn + "data." + cat), wn, info);
      load_wordnet_index((dn + "index." + cat), wn, info);
      load_wordnet_exc(dn, cat, wn, info);
    }



  void load_wordnet(const string& dn, Wordnet& wn, info_helper& info)
  {
    // vertex added in this order a n r v

    string fn = dn;

    cout << endl;
    cout << "### Loading Wordnet 3.0";
    boost::progress_display show_progress(4);
    boost::progress_timer t;

    load_wordnet_cat(dn, "adj", wn, info);
    ++show_progress;
    load_wordnet_cat(dn, "noun", wn, info);
    ++show_progress;
    load_wordnet_cat(dn, "adv", wn, info);
    ++show_progress;
    load_wordnet_cat(dn, "verb", wn, info);
    ++show_progress;
    cout << endl;

    stable_sort(wn.indexList.begin(), wn.indexList.end());
  }


} // endof namespace wn
