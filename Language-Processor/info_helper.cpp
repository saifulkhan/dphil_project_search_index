...
static std::map<int,int> preprocess_data(const std::string& fn)
{
  std::map<int,int> map;
  ...
  std::size_t ind = 0;
  //parse data line
  std::cout << "Debug: " << __PRETTY_FUNCTION__ << fn << "\n";
  while (file.getline(row, MAX_LENGTH)) {
    ...
    ind++;
    //std::cout << "Debug: " << __PRETTY_FUNCTION__ << "String: " << srow << "\n";
    //std::cout << "Debug: " << __PRETTY_FUNCTION__ << "<offset, ind> = <" << offset << ", " << ind << ">\n";
  }
...
const char* info_helper::symbols[info_helper::NB_SYMBOLS] = {
"!" ,  /// 0 Antonym
"@" ,  /// 1 Hypernym
...
void info_helper::update_pos_maps()
{
