...
string QueryProcessor::buildTreemap(const int x, const int y, const int width, const int height) {
    cout << __PRETTY_FUNCTION__ << endl;
    string jsonstring = m_treemap->buildTreemap(x, y, width, height);
    return jsonstring;
}


string QueryProcessor::zoomTreemap(const string path) {
    cout << __PRETTY_FUNCTION__ << endl;
    string jsonstring = m_treemap->zoomTreemap(path);
    return jsonstring;
}


string QueryProcessor::changeTreemapDepth(const int depth) {
    cout << __PRETTY_FUNCTION__ << endl;
    string jsonstring = m_treemap->changeTreemapDepth(depth);
    return jsonstring;
}
...
json_spirit::Array QueryProcessor::toJSONObject(vector<FileInfoX> result)
{
    ...
    for(vector<FileInfoX>::iterator it = result.begin(); it != result.end(); ++it)
    {
        ...
        pair<double, double> coordinate = m_treemap->coordinate(fileFullPath).center(); // centre of the tile

        // when the treemap is zoomed
        if(coordinate.first > 0 && coordinate.second > 0) {
            ...
            obj.push_back(json_spirit::Pair("x", coordinate.first));
            obj.push_back(json_spirit::Pair("y", coordinate.second));

            array.push_back(obj);
        } else {
           //cout << "rectF(x,y) = (" << rectF.x() << "," << rectF.y() << ")" << endl;
        }
    }
    return array;
}
