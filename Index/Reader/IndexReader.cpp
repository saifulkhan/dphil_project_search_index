...
void IndexReader::open(char *filepath) {
    ...
    cout << __PRETTY_FUNCTION__ << " Index location: " << filepath << endl;
   ...
   p_fdiInput=new IndexInput(p_fdi);
    docCount=p_fdiInput->readLong(); // First, write the total number of the first to write 0 overlying etc. back to the beginning and then finished
...
void IndexReader::getTermDocs(string &term, vector<HitDoc> &docVo)
{
    map<string,int64_t>::iterator it = m_termMap.find(term);
    ...
    if( it != m_termMap.end() ) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            map<int64_t, int64_t>::iterator delIt = m_delMap.find(docId);
...
void IndexReader::del(int64_t docid)
{
	...
	delCount++;
    m_delMap[docid] = docid;
//	map<string, vector<int64_t> >::iterator it=m_delMap.find(segName);
//	if(it!=m_delMap.end()){
